...
class OAuth1{
  ...
  public function __construct($ck, $cS, $oT, $oTS)
  {
      if(isset($ck)){
          ...
          throw new \Exception("Consumer key is not passed.");
      }

      if(isset($cS)){
          $this->consumerSecret = $cS;
      }else{
          throw new \Exception("Consumer Secret is not passed.");
      }

      if(isset($oT)){
          $this->oauthToken = $oT;
      }else{
          throw new \Exception("OAuth Token is not passed.");
      }

      if(isset($oTS)){
          $this->oauthTokenSecret = $oTS;
      }else{
          throw new \Exception("OAuth token Secret is not passed.");
      }
